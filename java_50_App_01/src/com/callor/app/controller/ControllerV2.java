...
public class ControllerV2 {
	...
	public static void main(String[] args) {
		...
		/*ServiceV2 에는 기본생성자가 없기때문에 
		 기본생성자를 호출하여 인스턴스를 생성 할 수 없다.
		 *그 이유는 ServiceV2 에서는 내부네서 사용하는 
		 *intList 를 초기화 하는 코드가 없고
		 *ServiceV2를 호출하는 외부에서 
		 *초기화된 List 를 매개변수로 주입해서 사용해야 하기 때문
		 *
		 *만약 기본생성자를 호출하여 객체를 만들게 되면
		 *intList 에서 Nullpointer Exteption 이 발생할 것이다
		 *그러한 이유로 기본생성자를 아예 호출하지 못하도록 
		 *만든다
		 *기본생성자에서 intList 를 초기화하는 코드를
		 *추가 할수 있지만, ServiceV2 클래스의 생성자가 복잡해 질수 있다
		 *
		 */

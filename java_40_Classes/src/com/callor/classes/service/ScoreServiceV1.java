...
public class ScoreServiceV1 {
	 ...
	 * 인스턴스변수들을 private 이 아니 protected 로 선언한다
	 * protected 는 private 의 성질을 가지면서
	 * 상속받은 클래스에서  상속받아 변수를 선언한것과 같은 효과흫 낸다
	 * 인스턴스변수도 상속이 된다.
	 */
	...
	public ScoreServiceV1(int length) {
		...
		for(index = 0 ; index < students.length ; index++) {
			int strNum = index +1;
			System.out.printf("%d 번 학생 이름(END: 중단) : ", index+1);
			// 키보드에서 문자열 입력받기
			String strName = scan.nextLine();
			// 이름대신 ENd 문자열을 입력하면 입렫받기를 중단한다.
			...
			System.out.printf("%d 번 국어점수 : >",strNum);

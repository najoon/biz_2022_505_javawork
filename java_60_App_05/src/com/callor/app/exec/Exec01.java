 ...
 * 0번 위치와 1~끝 위치까지 비교하면서 큰값은 오른쪽으로 작은값은 왼쪽으로 
 * 1 번 위치와 2~ 끝 위치까지 비교하면서 큰값은 오른쪽으로 작은 값은 왼쪽으로
 * 이 순회(반복) 을 전체 개수 많큼 반복실행하여 데이터를 정렬하는 알고리즘이다
 * 코드가 매우 단순해서  조금만 살펴보면 쉽게 이해가능.
